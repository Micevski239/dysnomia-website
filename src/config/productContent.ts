// Static content for product detail pages
// Edit this file to update the global content shown on all product pages

export const productContent = {
  deliveryAndReturns: `Free shipping on all orders within the United States.

International shipping is available. Shipping costs and delivery times vary by destination.

All artwork is carefully packaged to ensure safe delivery. Each piece is wrapped in acid-free tissue, cushioned with protective materials, and shipped in a sturdy box.

Returns are accepted within 14 days of delivery for items in original condition. Please contact us to initiate a return. Custom or commissioned pieces are final sale.`,

  support: `For questions about a specific artwork, sizing, or shipping, please contact us at hello@dysnomia.art

We typically respond within 24-48 hours.

If you're interested in commissioning a custom piece or have special requests, we'd love to hear from you.`,
};
